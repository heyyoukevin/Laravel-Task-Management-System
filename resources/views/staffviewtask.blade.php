...
<tr>
     ...
     <td><?php
     $un=\DB::select("select *from modules where id=?",[$userf->mid]);
     if(isset($un))
     {
        foreach($un as $u)
        {
    ...
    <td><a class="btn btn-success" href="/staffedittaskstatus/{{$userf->prjid}}/{{$userf->mid}}/{{$userf->id}}">UPDATE STATUS</a></td>
    @endif
    @if($userf->endt<$curdate)
