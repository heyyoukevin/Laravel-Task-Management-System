      ...
      <div class="row">
         <div class="col-lg-12">
            <div class="full text_align_center heading_s1">
                 <h2 style="width: 100%;text-align: center;">DELETE REUBRO PROJECTS</h2>
            </div>
         </div>
      </div>		
</section>
...
<fieldset>
<center>
<form action="/destroyproject/{{$user->id}}" method="POST" onSubmit="return check()">
{{ csrf_field() }}
<table>
...
<td>PROJECT NAME</td>
<td><input type="text" name="prjname" id="prjname" value="{{$user->prjname}}" required ></td>
...
<td>DESCRIPTION</td>
<td><textarea type="text" name="desc" id="desc" value="{{$user->desc}}" required ></textarea></td>

</tr><tr><td><br></td></tr>

<td>CLIENT NAME</td>
<td><input type="text" name="clname" id="clname" value="{{$user->clname}}" required ></td>
...
<button type="submit" style="background-color:black;color:white">DELETE</button>&nbsp; &nbsp;<a href="/viewstaff" class="btn btn-outline">&#x1F519;</a>
</fieldset>
</center>
</form>
...
<script type="text/javascript">
    ...
    function check()
    {
    if(!document.getElementById("clname").value.match(letters))
        {
              
            alert('Please input alphabet characters only,enter client name properly');
            return false;
        }
    else if(!document.getElementById("hr").value.match(numbers))
        {
